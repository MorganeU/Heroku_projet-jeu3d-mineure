...
function createScene() {
    ...
    tank.ellipsoid = new BABYLON.Vector3(5, 5, 5); // ellipse autour du tank pour éviter de s'enfoncer dans le sol 
    let box = createBox(scene);
    let bouton = createButton(scene, tank);
    // second parameter is the target to follow
    scene.followCamera = createFollowCamera(scene, tank);
...
function createButton(scene, tank) {
...
function createTank(scene) {
    let tank = new BABYLON.MeshBuilder.CreateSphere("heroTank", { diameter: 10, segments: 40 }, scene);
    ...
    tank.move = () => {
        ...
        if (scene.inputStates.monter) {
            tank.position.y += 0.3;
        }
        // le tank va redescendre
        if (scene.inputStates.descendre) {
            tank.position.y -= 0.3;
        }
        // tire des lasers
        if (scene.inputStates.laser) {
            tank.fireLasers();
        }
    }
